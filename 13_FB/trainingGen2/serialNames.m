%
% return serial port with given name start
% WORKS ONLY WITH WINDOWS
% SPE 5/2018
%
function port = serialNames(nameBase)
port = '';  % default to not present
...
%     end
    ...
    for i = 1:numel(vals)
        %if contains(vals{i},nameBase) == 1
        if numel(strfind(vals{i},nameBase)) ~= 0
            % get COM number in next line and we are done
            str = vals{i+1};
            % com identifier is in parens
            split = regexp(str,'(','split');
            tail = split{end};
            split = regexp(tail,')','split');
            if numel(split) == 2
                port = split{1};
            end
            return;
