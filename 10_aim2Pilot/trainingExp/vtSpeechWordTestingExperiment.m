...
function vtSpeechWordTestingExperiment(name, exptdesign)
    ...
    drawAndCenterText(w,'\nAim 2 VT speech testing',1)
    
    words_cfg %load words config file 
    load VTspeechStim_words.mat;
...
%    each block trains a phonetic feature
 %   randomize order of blocks
    ...
    for iBlock=1:exptdesign.numSessions %how many blocks to run this training session
        ...
        if strcmp(feature_block{iBlock},'voicing'), word_pairs = repmat(word_pairs_voicing,1,4);
        elseif strcmp(feature_block{iBlock},'manner'), word_pairs = repmat(word_pairs_manner,1,3);
        elseif strcmp(feature_block{iBlock},'place'), word_pairs = repmat(word_pairs_place,1,4);
        end
        ...
        for iTrial=1:exptdesign.numTrialsPerSession
            ...
            target = word_pairs{iTrial}(1);
            foil = word_pairs{iTrial}(2);
            target_ind = find(strcmp(list_words, char(target)));
            ...
            r = randperm(2); %random permutation of 1 and 2 to randomize which order of correct/incorrect word in prompt 
            if r(1) == 1
                word1 = target;
                word2 = foil;
                correctResp = 1;
            ...
            [nx, ny, bbox] = DrawFormattedText(w, [char(word1) ' or ' char(word2) '?\n'], 'center', 'center', 1);
            ...
            WaitSecs(2);
            stimGenPTB('load',VTspeechStim{2,target_ind},VTspeechStim{1,target_ind});
            stimGenPTB('start');
...
%                     'Click mouse to continue' ],1)
                drawAndCenterText(w, ['Click mouse to continue when you are ready' ],1)
...
%                     'You have completed this training session.  Thank you for your work!' ],1)
                drawAndCenterText(w, ['All done!' ],1)
