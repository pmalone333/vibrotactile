...
function trialOutput = oneBackTaskExperiment(name,exptdesign)
%dbstop if error;
try
%     dbstop if error;
    ...
    for iBlock = 1:numBlocks %how many blocks to run this training session
        ...
        for i=1:length(stimOrder)
            ...
            if strcmp(stimOrder{i}(1),stimOrder{i}(2)), accuracy = 1; else accuracy = 0; end
            WaitSecs(1)
        end
...
%         end
        ...
        trialOutput(iBlock,1).stimuli                 = stimOrder;
