%
% modeled off of: NSF130201 - experiment for CV EEG testing with training
%
% SPE 1/28/13, 6/27/13
%
function rslt = NSF130201(subjID, session, controlFile, comment)
...
try
    ...
    rslt = 'err: "NSF130201 parameter file parse';
    return;
end
...
%     % initialize audio system to calibrated volume
%     try
%         eval('NSF130201cal');
%         setAudioVariables(AudioControls,'volume','volume',snd_level);
%     catch
%         rslt = 'err: could not load calibration file\n';
%     end
...
%        Screen('Blendfunction', ...)
    ...
    movie = Screen('OpenMovie', win, videoFile);
    ...
    [y, freq, ~] = wavread(audioFile);
    fprintf('loaded wave file, sampling rate is %d\n',freq);
    wavedata = y';
    ...
    % setup for first bell output
    if bell_frequency ~= 0
        bell_stimNum = round(bell_frequency(1)...
    ...
    %
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    fprintf('starting NSF130201, session=%s, subj=%s\n',session, subjID);
